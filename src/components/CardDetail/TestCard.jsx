  ...
  const callAppiEpisode = (episode) => {
    ...
    fetch(
      `https://rickandmortyapi.com/api/episode/${
        index.length > 4 ? index.slice(0, 4) : index
      }`
    )
      .then((res) => res.json())
      .then((data2) => {
  ...
  return (
    <div className="container-details mt-8">
      <div className="flex flex-col justify-center">
        <Link to="/">
          <div className="flex absolute flex-column items-center gap-2 realtive">
            <AiOutlineArrowLeft style={{ fontWeight: "bolder" }} />
            <h1 className="font-bold">Go Back</h1>
          </div>
        </Link>
        <div className="container-img-name flex flex-col justify-center items-center justify-center mt-8">
          <img className="rounded-full" src={image} alt="imagen" />
          <h1 className="font-bold text-xl">{name}</h1>
        </div>
        <div className="row-2 ml-8 gap-8 flex flex-row justify-center mt-6 mb-4">
          ...
          <div className="col-2 w-96">
            <p className="font-bold text-gray-700 mb-8">Episodes</p>
            {episodeInfo !== undefined && episodeInfo.length > 0 ? (
              episodeInfo.map((episode) => {
                return (
                  <div key={episode?.id}>
                    <p className="font-bold ">{episode?.episode}</p>
                    <p className="text-gray-600">{episode?.name}</p>
                    <p className="text-gray-600 pb-4 border-b-2 border-stone-100 items-center">
                      {episode?.air_date}
                    </p>
                  </div>
                );
              ...
              <div>
                <p className="font-bold ">{episodeInfo?.episode}</p>
                <p className="text-gray-600">{episodeInfo?.name}</p>
                <p className="text-gray-600 pb-4 border-b-2 border-stone-100 items-center">
